...
import { authMiddleware, optionalAuthMiddleware, TokenPayload } from '../auth/jwt.js';

const postsApp = new Hono<{ Variables: { user?: TokenPayload } }>();
...
postsApp.get('/', optionalAuthMiddleware, async (c) => {
  ...
  if (search) {
    ...
    conditions.push(or(like(posts.title, searchPattern), like(posts.summary, searchPattern), like(posts.content, searchPattern)) as any);
  }
...
postsApp.post('/', authMiddleware, zValidator('json', createPostSchema), async (c) => {
  ...
  const baseSlug = (slugify as any).default ? (slugify as any).default(data.title, { lower: true, strict: true }) : (slugify as any)(data.title, { lower: true, strict: true });
